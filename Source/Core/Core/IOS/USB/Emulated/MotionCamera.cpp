...
{
  NOTICE_LOG_FMT(IOS_USB, "[{:04x}:{:04x}] FL: GetInterfaces: config={:02x}", m_vid, m_pid, config);
  return s_interface_descriptor;
}

std::vector<EndpointDescriptor> MotionCamera::GetEndpoints(u8 config, u8 interface, u8 alt) const
{
  NOTICE_LOG_FMT(IOS_USB, "[{:04x}:{:04x}] FL: GetEndpoints: config={:02x} if={:02x} alt={:02x}", m_vid, m_pid, config, interface, alt);
  ...
  //NOTICE_LOG_FMT(IOS_USB, "[{:04x}:{:04x}] FL: GetEndpoints: addr={:02x}", m_vid, m_pid, ret.size() > 0 ? ret.at(0).bEndpointAddress : 0);
  return ret;
}
...
{
  NOTICE_LOG_FMT(IOS_USB, "[{:04x}:{:04x}] FL: GetNumberOfAltSettings: interface={:02x}", m_vid, m_pid, interface);
  return (interface == 1) ? 7 : 1;
}

int MotionCamera::SetAltSetting(u8 alt_setting)
{
  NOTICE_LOG_FMT(IOS_USB, "[{:04x}:{:04x}] FL: SetAltSetting: alt_setting={:02x}", m_vid, m_pid, alt_setting);
...
int MotionCamera::SubmitTransfer(std::unique_ptr<IsoMessage> cmd)
{
  ...
  for (std::size_t i = 0; i < cmd->num_packets; i++)
  {
    ...
    //NOTICE_LOG_FMT(IOS_USB, "[{:04x}:{:04x}] IsoMessage FID={:02x} EOF={:02x} image_pos={:02x}", m_vid, m_pid, uvc_header.frameId, uvc_header.endOfFrame, image_pos);
...
{
  NOTICE_LOG_FMT(IOS_USB, "SetData length={}", length);
  m_image_size = length;
